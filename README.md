# 🏛️ LNN-Nivaaran — Next-Gen Smart Citizen Grievance & Predictive Governance Platform
...
LNN-Nivaaran implements an autonomous nodal agent follow-up simulation engine that performs dynamic background audits, status pings, and notification updates over a 5-day lifecycle:
...
LNN-Nivaaran runs an autonomous spatial clustering algorithm in the local storage data layer to identify regional systemic failures:
...
LNN-Nivaaran leverages standard fallbacks to run smoothly in offline/demo mode, but activates full multimodal capability when configured with a `GEMINI_API_KEY`:
...
graph TD
    ...
    B -->|Gemini 2.5 Flash Heuristics| C{LNN-Nivaaran API}
...
```bash
git clone https://github.com/theabhishek4u/LNN-Nivaaran.git
cd lnn-nivaaran
```
...
To add new departments or tweak classification keywords, update [src/data/departments.ts](file:///c:/My%20Project/Agentic%20Premier%20League%20(APL)/lnn-nivaaran/src/data/departments.ts). The Gemini Vision engine automatically inherits the updated system prompts at runtime.

### Tweak Spatial Clustering Rules
To change the hotspot clustering threshold (default: `>= 2` duplicate active reports), adjust the parameter inside [src/lib/complaints.ts](file:///c:/My%20Project/Agentic%20Premier%20League%20(APL)/lnn-nivaaran/src/lib/complaints.ts#L126).
